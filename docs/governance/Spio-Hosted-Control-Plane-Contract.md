...
**Purpose:** Freeze the frontend/backend HTTP contract used by hosted workspaces so `vityo-nightly`, the future `spio` control console, and backend services can develop independently against a versioned native JSON package.
