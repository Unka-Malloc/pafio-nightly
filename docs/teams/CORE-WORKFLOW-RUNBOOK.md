...
9. Keep registry package identity, registry object paths, HTTP read limits, and tar pre-extraction checks aligned between the native client and Python registry v2 helpers.
10. Preserve offline package operation: cache reads must not require platform connectivity once metadata and artifacts are available locally.
...
3. High: binary/build execution routing, source-build fetch/build semantics, registry fetch/extract trust policy, offline package guarantees, platform compatibility semantics, or checkpoint entrypoint change. Coordinate with Docs / Delivery, Registry / Publish, and Styio / Contracts.
