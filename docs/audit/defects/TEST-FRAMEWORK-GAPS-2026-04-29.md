...
| SPIO-TEST-003 | Medium | Cross-repo contract | The CI workflow checks out `Unka-Malloc/styio-view` at fixed `nightly`, while the `styio-nightly` sibling is branch-aware. PRs against `ai-dev` can therefore validate against a stale or mismatched view contract. | `.github/workflows/local-ci-gate.yml` pins `styio-view` checkout to `ref: nightly`. | Use the same branch/ref resolution strategy for sibling repos, or record a reviewed exception explaining why `styio-view:nightly` is the intended compatibility target. |
...
- Fix or document sibling checkout branch policy for `styio-view`.
