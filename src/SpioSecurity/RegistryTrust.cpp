...
json
LoadTrustStore(const fs::path &spio_home) {
...
void
WriteTrustStore(const fs::path &spio_home, const json &payload) {
  const fs::path store_path = spio::RegistryTrustStorePath(spio_home);
  spio::AtomicWriteFile(store_path, payload.dump(2) + '\n');
}

spio::RegistryTrustPin
...
EnforceDescriptorValidityWindow(const json &signed_payload) {
  ...
  if (!issued.has_value() || !expiry.has_value()) {
    throw spio::FetchError("registry trust descriptor issued_at/expires must be UTC timestamps");
  }
  const auto now = std::chrono::system_clock::now();
  constexpr auto kSkew = std::chrono::minutes{5};
  if (*issued > now + kSkew) {
    throw spio::FetchError("registry trust descriptor issued_at is in the future");
  }
  if (*expiry < now - kSkew) {
    throw spio::FetchError("registry trust descriptor has expired");
  }
  if (*expiry < *issued) {
    throw spio::FetchError("registry trust descriptor expires before issued_at");
  }
}

std::unordered_map<std::string, std::string>
LoadDescriptorSignerKeys(const fs::path &spio_home) {
  std::unordered_map<std::string, std::string> keys;
  if (const char *env_keys = std::getenv("SPIO_DESCRIPTOR_SIGNER_KEYS"); env_keys != nullptr && *env_keys != '\0') {
    try {
      const json payload = json::parse(env_keys);
      if (!payload.is_object()) {
        throw spio::FetchError("SPIO_DESCRIPTOR_SIGNER_KEYS must be a JSON object of keyid->PEM");
      }
      for (auto it = payload.begin(); it != payload.end(); ++it) {
        if (!it.value().is_string()) {
          throw spio::FetchError("SPIO_DESCRIPTOR_SIGNER_KEYS values must be PEM strings");
        }
        keys[it.key()] = it.value().get<std::string>();
      }
    }
    catch (const json::parse_error &) {
      throw spio::FetchError("SPIO_DESCRIPTOR_SIGNER_KEYS is not valid JSON");
    }
  }

  const fs::path signers_path = spio::RegistryTrustRoot(spio_home) / "descriptor-signers.json";
  if (fs::exists(signers_path)) {
    try {
      ...
      if (!payload.is_object() || !payload.contains("keys") || !payload["keys"].is_object()) {
        throw spio::FetchError("descriptor-signers.json must contain a keys object");
      }
      ...
      throw spio::FetchError("descriptor-signers.json is not valid JSON");
    }
  }
  return keys;
}
...
VerifyDescriptorSignatures(const json &envelope, const std::unordered_map<std::string, std::string> &signer_keys) {
  if (!envelope.contains("signed") || !envelope["signed"].is_object()) {
    throw spio::FetchError("registry trust descriptor must contain a signed object");
  }
  if (!envelope.contains("signatures") || !envelope["signatures"].is_array() || envelope["signatures"].empty()) {
    throw spio::FetchError("registry trust descriptor is unsigned");
  }
  if (signer_keys.empty()) {
    throw spio::FetchError(
      ...
      "SPIO_DESCRIPTOR_SIGNER_KEYS"
    );
  }
  const std::string message = spio::CanonicalJsonBytes(envelope["signed"]);
  int valid = 0;
  for (const json &signature : envelope["signatures"]) {
    ...
    if (spio::Ed25519VerifyPem(it->second, message, sig)) {
      ++valid;
    }
  }
  if (valid < 1) {
    throw spio::FetchError("registry trust descriptor signature is invalid or signer keyid is unknown");
  }
}
...
LoadRegistryTrustPins(const fs::path &spio_home) {
...
ResolveRegistryTrustPin(const fs::path &spio_home, const std::string &registry_root) {
...
ImportRegistryTrustDescriptor(
  ...
  if (is_envelope) {
    const auto signer_keys = LoadDescriptorSignerKeys(spio_home);
  ...
  const FileLockGuard trust_lock =
    AcquireFileLock(RegistryTrustRoot(spio_home), FileLockScope::kTrust);
