...
#if defined(_WIN32)
    throw spio::FetchError("ed25519 verification is not supported on Windows");
#else
    std::string pattern = (fs::temp_directory_path() / "spio-ed25519-XXXXXX").string();
    std::vector<char> buffer(pattern.begin(), pattern.end());
    buffer.push_back('\0');
    const char *created = ::mkdtemp(buffer.data());
    if (created == nullptr) {
      throw spio::FetchError("failed to create secure temporary ed25519 verify directory");
    }
    path_ = created;
#endif
  }
...
WriteBinaryFile(const fs::path &path, std::string_view content) {
  ...
  if (!out) {
    throw spio::FetchError("failed to open temporary ed25519 verify file: " + path.string());
  }
  out.write(content.data(), static_cast<std::streamsize>(content.size()));
  if (!out.good()) {
    throw spio::FetchError("failed to write temporary ed25519 verify file: " + path.string());
  }
}
