...
fs::path
RegistryObjectCachePath(const fs::path &spio_home, const std::string &registry_root, const std::string &relative_path) {
  return spio::RegistryIndexCacheRoot(spio_home) / spio::Sha256Text(registry_root) / NormalizeRelativeRegistryPath(relative_path, "registry cache object path");
}

fs::path
BlobCachePath(const fs::path &spio_home, const std::string &sha256) {
  return spio::RegistryBlobCacheRoot(spio_home) / sha256.substr(0U, 2U) / sha256.substr(2U, 2U) / (sha256 + ".tar");
}

fs::path
CheckoutPath(const fs::path &spio_home, const std::string &package_name, const std::string &version, const std::string &sha256) {
...
ValidateTarListingPaths(const fs::path &blob_cache_path) {
  ...
  if (path_listing.exit_code != 0) {
    throw spio::CacheError(
      "failed to list registry artifact '" + blob_cache_path.string() + "': " + spio::DescribeProcessFailure(path_listing)
    );
  }
  ...
  if (verbose_listing.exit_code != 0) {
    throw spio::CacheError(
      "failed to inspect registry artifact '" + blob_cache_path.string() + "': " + spio::DescribeProcessFailure(verbose_listing)
    );
  }
...
MaterializeRegistryPackage(
  ...
  if (IsHttpRegistry(security.registry_root)) {
    ...
    if (!trust_pin.has_value()) {
      throw FetchError(
        "remote registry is not trusted: import a platform registry descriptor with 'spio registry trust import' before fetching " + security.registry_root
      );
    }
