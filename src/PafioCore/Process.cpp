...
std::string
DescribeProcessContext(std::string context) {
  if (context.empty()) {
...
void
SetNonBlocking(int fd, const std::string &context) {
...
void
ApplyChildEnvironment(const pafio::ProcessRequest &request) {
...
void
WriteChildSetupError(const std::string &message) {
  static_cast<void>(write(STDERR_FILENO, message.c_str(), message.size()));
}

void
KillChildProcess(const pid_t child, const bool terminate_process_group) {
...
void
ReapChildProcessNoThrow(const pid_t child) {
  int status = 0;
...
ProcessResult
RunProcessChecked(const ProcessRequest &request) {
  const std::string context = DescribeProcessContext(request.error_context);

  int stdin_pipe[2] = {-1, -1};
  int stdout_pipe[2] = {-1, -1};
  int stderr_pipe[2] = {-1, -1};
  if (!request.stdin_text.empty() && socketpair(AF_UNIX, SOCK_STREAM, 0, stdin_pipe) != 0) {
  ...
  try {
    ...
    while (stdout_open || stderr_open) {
      ...
      if (request.timeout.has_value()) {
        ...
        const auto remaining = (*request.timeout - elapsed).count();
        timeout_ms = remaining > INT_MAX ? INT_MAX : static_cast<int>(remaining);
      }
