...
std::filesystem::path
LockFileFor(const std::filesystem::path &store_path, spio::FileLockScope scope) {
  const char *suffix = "project";
  switch (scope) {
    case spio::FileLockScope::kProject:
      suffix = "project";
      break;
    case spio::FileLockScope::kCache:
      suffix = "cache";
      break;
    case spio::FileLockScope::kTrust:
      suffix = "trust";
      break;
  }
  return store_path / (std::string(".spio-") + suffix + ".lock");
}
