...
std::string ResolveOnPath(const std::string &name)
{
  ...
  {
    throw spio::FetchError("PATH is empty; cannot resolve tool '" + name + "'");
  }
  ...
  throw spio::FetchError("failed to resolve absolute path for tool '" + name + "' on PATH");
}
...
const std::string &ResolveTool(const char *env_name, const char *default_name, std::string &slot)
{
  ...
  if (const char *override_path = std::getenv(env_name); override_path != nullptr && *override_path != '\0')
  {
    ...
    {
      throw spio::FetchError(std::string(env_name) + " must be an absolute path");
    }
    if (!IsExecutableFile(path))
    {
      throw spio::FetchError(std::string(env_name) + " does not point to an executable: " + override_path);
    }
...
{
  return ResolveTool("SPIO_CURL", "curl", g_curl);
}

const std::string &ResolvedGitPath()
{
  return ResolveTool("SPIO_GIT", "git", g_git);
}

const std::string &ResolvedTarPath()
{
  return ResolveTool("SPIO_TAR", "tar", g_tar);
}

const std::string &ResolvedOpenSslPath()
{
  return ResolveTool("SPIO_OPENSSL", "openssl", g_openssl);
}
