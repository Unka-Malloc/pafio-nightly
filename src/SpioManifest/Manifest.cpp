...
QuoteTomlKey(const std::string &value) {
  ...
  return spio::QuoteTomlString(value);
}
...
void
EmitDependencySection(std::ostringstream &out, const std::string &section_name, const std::vector<spio::Dependency> &dependencies) {
  ...
  for (const spio::Dependency &dependency : ordered) {
    ...
    auto emit_field = [&](const std::string &label, const std::string &value)
    {
      ...
      out << label << " = " << spio::QuoteTomlString(value);
      first_field = false;
    };
...
SerializeManifestCanonical(const ManifestDocument &manifest) {
  ...
  if (manifest.package.has_value()) {
    ...
    out << "\n[package]\n";
    out << "name = " << spio::QuoteTomlString(package.name) << "\n";
    out << "version = " << spio::QuoteTomlString(package.version) << "\n";
    ...
    if (package.lib.has_value()) {
      out << "\n[lib]\n";
      out << "path = " << spio::QuoteTomlString(package.lib->path) << "\n";
    }
    ...
    if (!package.bins.empty()) {
      ...
      for (const BinTarget &bin : bins) {
        out << "\n[[bin]]\n";
        out << "name = " << spio::QuoteTomlString(bin.name) << "\n";
        out << "path = " << spio::QuoteTomlString(bin.path) << "\n";
      }
    }
    ...
    if (!package.tests.empty()) {
      ...
      for (const TestTarget &test : tests) {
        out << "\n[[test]]\n";
        out << "name = " << spio::QuoteTomlString(test.name) << "\n";
        out << "path = " << spio::QuoteTomlString(test.path) << "\n";
      }
    }
  ...
  if (manifest.workspace.has_value()) {
    ...
    out << "resolver = " << spio::QuoteTomlString(workspace.resolver) << "\n";
  }
