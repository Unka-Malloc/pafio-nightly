...
SerializeLockfileCanonical(const LockfileDocument &lockfile) {
  ...
  out << "lock-version = 1\n";
  out << "\n[metadata]\n";
  out << "generated-by = " << spio::QuoteTomlString(lockfile.generated_by) << "\n";
  out << "resolver = " << spio::QuoteTomlString(lockfile.resolver) << "\n";
  ...
  for (const LockPackage &package : packages) {
    out << "\n[[package]]\n";
    out << "id = " << spio::QuoteTomlString(package.id) << "\n";
    out << "name = " << spio::QuoteTomlString(package.name) << "\n";
    out << "version = " << spio::QuoteTomlString(package.version) << "\n";
    out << "source-kind = " << spio::QuoteTomlString(package.source_kind) << "\n";
    if (package.source_kind == "git") {
      out << "git = " << spio::QuoteTomlString(package.git.value_or("")) << "\n";
      out << "rev = " << spio::QuoteTomlString(package.rev.value_or("")) << "\n";
    }
    else if (package.source_kind == "registry") {
      out << "registry = " << spio::QuoteTomlString(package.registry.value_or("")) << "\n";
      out << "sha256 = " << spio::QuoteTomlString(package.sha256.value_or("")) << "\n";
    }
    ...
    for (size_t index = 0; index < dependencies.size(); ++index) {
      ...
      out << spio::QuoteTomlString(dependencies[index]);
    }
    out << "]\n";
  }
