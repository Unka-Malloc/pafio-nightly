...
ParseStringArray(const toml::table &table, std::string_view key, std::string_view context) {
...
void
ValidateSourceKind(const std::string &source_kind) {
...
void
ValidateSha256(const std::string &sha256, std::string_view context) {
...
LockfileDocument
LoadLockfile(const fs::path &lockfile_path) {
  toml::table doc;
  try {
...
std::string
SerializeLockfileCanonical(const LockfileDocument &lockfile) {
  std::ostringstream out;
  out << "lock-version = 1\n";
  out << "\n[metadata]\n";
  out << "generated-by = " << pafio::QuoteTomlString(lockfile.generated_by) << "\n";
  out << "resolver = " << pafio::QuoteTomlString(lockfile.resolver) << "\n";
  ...
  for (const LockPackage &package : packages) {
    out << "\n[[package]]\n";
    out << "id = " << pafio::QuoteTomlString(package.id) << "\n";
    out << "name = " << pafio::QuoteTomlString(package.name) << "\n";
    out << "version = " << pafio::QuoteTomlString(package.version) << "\n";
    out << "source-kind = " << pafio::QuoteTomlString(package.source_kind) << "\n";
    if (package.source_kind == "git") {
      out << "git = " << pafio::QuoteTomlString(package.git.value_or("")) << "\n";
      out << "rev = " << pafio::QuoteTomlString(package.rev.value_or("")) << "\n";
    }
    else if (package.source_kind == "registry") {
      out << "registry = " << pafio::QuoteTomlString(package.registry.value_or("")) << "\n";
      out << "sha256 = " << pafio::QuoteTomlString(package.sha256.value_or("")) << "\n";
    }
    ...
    for (size_t index = 0; index < dependencies.size(); ++index) {
      ...
      out << pafio::QuoteTomlString(dependencies[index]);
    }
    out << "]\n";
  }
